...
use App\academic_syllabus_model;
...
class ischool_report extends Controller
{
    ...
    {
        if(Entrust::can('APPLICANT_STUDNETS REPORT'))
        {
    ...
    {
        if(Entrust::can('TEACHER REPORT'))
        {
            return view('admin.Teacher_Staff.teacher_report',['teacher_report'=>teacher_model::where('status','teacher')->orderBy('sort_index', 'ASC')->get()]);
        }
        else
        {
            abort(403);
    ...
    {
        if(Entrust::can('ATTENDENCE_REPORT'))
        {
    ...
    {
        if(Entrust::can('TABULATION_SHEET'))
        {
