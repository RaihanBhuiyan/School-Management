...
@section('content')
    ...
    @if(isset($zoom_error))
        <div class="container">
            ...
            <p title="Transport Details">{{Session::get('school.system_name')}}( {{Session::get('school.school_eiin')}}</p> <br/> <!-- Transport Details -->
        </div>
        <div class="tab-content"> <!-- Transport List Report  -->
            <div class="widget-box">
                ...
                <div class="widget-content text-center">
                    ...
                    <a class="btn btn-primary" href="#" onclick="location.reload()"><h1>Reload</h1></a>
                </div>
            </div>
        </div>
    @else
        ...
        <div id="Vue_component_wrapper">
            ...
            <ul class="nav nav-tabs">
                <li class="active"><a data-toggle="tab" href="#home"><i class="fa fa-bars" aria-hidden="true"></i>Live Classes</a></li>

                @permission('live_class')<li><a data-toggle="tab" href="#menu1"><i class="fa fa-plus-circle" aria-hidden="true"></i>Add Live Class</a></li>@endpermission
                            ...
                            <table class="table table-bordered data-table font_my">
                                ...
                                @foreach($live_classes as $key => $class)
                                    <tr class="gradeX">
                                        <td>{{$key+1}}</td>
                                        <td>{{$class->topic}}</td>
                                        <td>{{(array_key_exists($class->teacher_id, $teacher_data)) ? $teacher_data[$class->teacher_id]->name : ''}}</td>
                                        {{--                                    <td>{{$class->subject}}</td>--}}
                                        ...
                                        <td class="center start_class">
                                            ...
                                            @if($current_time > $end_time)
                                                <a>---</a>
                                            @else
                                                <a  target="_blank" href="{{$class->start_url}}" class="btn btn-primary" style="margin-top:5px; margin-left:5px;display: inline-block;white-space: nowrap;margin-bottom:5px">JOIN WITH APP</a>
                                                <a style="padding:4px"  target="_blank" id="{{$class->id}}" href="#" class="btn-primary start_button" >START</a>
                                            @endif

                                        </td>
                                        <td class="center">
                                            <button style="padding:4px;height:26px;" class="btn-success" style="padding: 5px" @click="OpenFileModal({{$class->id}})">Files</button>
                                        </td>
                                        <td class="center">
                                            <div class="display_status">
                                                ...
                                                {{Form::close()}}
                                                <button style="padding:4px;height:26px;" class="btn-danger" @click="DeleteClass({{$class->id}})" style="padding: 5px">Delete</button>
                                            </div>
                                        </td>
                                    </tr>
                                @endforeach
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <div class="col-md-12">
                                                <form method="post" action="{{url('update_class_file')}}" enctype="multipart/form-data">
                                                    <div class="control-group">
                                                        {{Form::label('Add Course Materials','',[])}}
                                                        {{@csrf_field()}}
                                                        <div class="controls" style="margin: 20px 0" v-for="(file, index) in Files">
                                                            <input name="class_id"  type="hidden" v-model="class_id">
                                                            <input type="text" required placeholder="File Name" name="file_name[]" class="form-control">
                                                            <input style="height: 33px; line-height: 34px;background: #ddd;margin-top: -10px;" type="file" name="files[]">
                                                            <a v-if="index === Files.length-1" @click="AddRow()" class="btn btn-primary">Add Row</a>
                                                            <a v-if="index+1 !== Files.length" @click="DeleteRow(index)" class="btn btn-warning">Remove</a>
                                                        </div>
                        ...
                        <div class="widget-box">
                            ...
                            <div class="widget-content nopadding">
                                ...
                                <div class="control-group">
                                    ...
                                    <div class="controls">
                                        <input id="datepicker"  autocomplete="off"  title="description" name="start_date" type="text">
                                        <input id="time1" class="timepicker" autocomplete="off"  name="start_time" type="text">
                                    </div>
                                </div>
                                <div class="control-group">
                                    <label for="description" class="control-label">End Time</label>
                                    <div class="controls">
                                        <input id="datepicker2"  autocomplete="off"  title="description" name="end_date" type="text">
                                        <input id="time2" class="timepicker"  autocomplete="off"   name="end_time" type="text">
                                    </div>
                                </div>
                                <div class="control-group">
                                    ...
                                    <div class="controls">
                                        ...
                                        <select name="class_name" id="class_name" @change="ClassNameChange('class_name')">
                                            ...
                                            @foreach($manage_class as $manage_class_list)
                                                <option value="{{$manage_class_list->class_name}}">{{$manage_class_list->class_name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="multiple" v-for="(input, in_index) in InputRowList">
                                    <div class="control-group">
                                        ...
                                        <div class="controls">
                                            ...
                                            <a v-if="in_index === InputRowList.length-1" class="btn btn-primary" @click="AddInputRow()">Add Row</a>
                                            <a v-if="in_index+1 !== InputRowList.length" class="btn btn-warning" @click="DeleteInputRow(in_index)">Delete Row</a>
                                        </div>
                                    </div>
                                </div>
                                <div class="control-group">
            ...
            methods: {
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            ...
                            $.each(response.data, function (i,v) {
                                html += '<tr><td>';
                                html += v.file_name += '</td>';
                                html += '<td class="center"><a class="btn btn-success" target="_blank" href="'+baseUrl+'/'+v.file_path+'">Download</a></td>';
                                html += '<td class="center"><a class="btn btn-danger" href="'+baseUrl+'/delete_class_file/'+v.id+'">Delete</a></td><tr>';
                            });
                            _this.html = html;
                        },
                        error: function (jqXHR, textStatus, errorThrown) {
                ...
                ClassNameChange : function (id) {
                    ...
                    $.ajax({
                        ...
                        type:"post",
                        data:{'class_name':class_name,'_token': '{{@csrf_token()}}'},
                        success:function(data) {
