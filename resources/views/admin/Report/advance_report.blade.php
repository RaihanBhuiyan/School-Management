...
@section('content')
    <div class="container">
        ...
        <!-- Tab Heading  -->
        <p title="Transport Details">{{Session::get('school.system_name')}}( {{Session::get('school.school_eiin')}} ) Daily Attandance's Report
        </p><br/>
        <!-- Transport Details -->
        ...
        <table style="margin-top: 4%; margin-left: 3%" class="">
            ...
            <tr>
                ...
                <td>{{Form::select('department',[''=>'--select class--'],null,['style'=>'width: 85px','id'=>'department'])}}</td>
                <td>{{Form::select('section',[''=>'--select section--'],null,['style'=>'width: 85px','id'=>'section'])}}</td>
                <td>{{Form::text('phone_no','',['style'=>'width: 85px','id'=>'phone_no'])}}</td>
                <td>{{Form::text('parent_name','',['style'=>'width: 85px','id'=>'parent_name'])}}</td>
                <td>{{Form::select('student_type',[''=>'--select--','Regular'=>'Regular','Irregular'=>'Irregular'],null,['style'=>'width: 85px','id'=>'student_type'])}}</td>
                <td>{{Form::select('gender',[''=>'--select--','Male'=>'Male','Female'=>'Female'],null,['style'=>'width: 85px','id'=>'gender'])}}</td>
          ...
          <tr>
              <td rowspan="2">
                ...
                <br>
                 ...
                 <br>
                <h4 style="margin-left: 320px;margin-top: -30px;">{{Session::get('school.address')}}, Postal Code-{{Session::get('school.postal_code')}}</h4>
              </td>
          </tr>
        </table>
      </div>
                    ...
                    success:function(data){
                        ...
                        if(data[0])
                        {
                            ...
                            for(var i=0;i<=data.length;i++)
                            {
                                ...
                                $("#department").append("<option>"+data[i].department_name+"</option>");
                            }
                        }
                        else {
                    ...
                    success:function(data){
                        ...
                        if(data[0])
                        {
                            ...
                            for(var i=0;i<=data.length;i++)
                            {
                                ...
                                $("#section").append("<option>"+data[i].section_name+"</option>");
                            }
                        }
                        else {
