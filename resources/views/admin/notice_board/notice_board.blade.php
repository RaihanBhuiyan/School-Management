	...
	<div class="container">
 		...
 		<p title="Transport Details">Notice Details</p> <!-- Transport Details -->
			  ...
			  <ul class="nav nav-tabs">
			    <li class="active"><a data-toggle="tab" href="#home"><i class="fa fa-bars" aria-hidden="true"></i> Notice List</a></li>
                                                ...
                                                else
                                                {
                                                 ...
                                                 @endphp</td>
                                                  ...
                                                  {{Form::close()}}
                                                   ...
                                                   {{Form::open(['url'=>"/notice_board/$all_data->notice_title" ,'method'=>'GET'])}}
	...
	<!--  Parent Wise Data add Part Start -->
	 ...
	 <!-- parents wise Data add part End  -->
		 ...
		 <div class="control-group">
			 ...
			 <div class="controls">
         {{Form::textarea('Notice','',['col'=>'20','rows'=>'4','title'=>'Notice'])}}
			 </div>
		 </div>
		  <div class="control-group">
             {{Form::label('files','Notice File',['class'=>'control-label','title'=>'queston_file'])}}
           <div class="controls">
              {{Form::file('files')}} <font color="red">PDF File Only Upload Here</font>
                </div>
            </div>
       ...
       $(".teacher_name").unbind().change(function(){
           var teacher_name=$(this).val();
            $.ajax({
                 ...
                 type: "post",
                 data:{'teacher_name':teacher_name,'_token':$('input[name=_token]').val()},
                 success:function(data)
                 {
       ...
       $(".pw_parents_name").unbind().change(function(){
            ...
            var parents_name=$(this).val();
             $.ajax({
                 ...
                 type: "post",
                 data:{'parents_name':parents_name,'_token':$('input[name=_token]').val()},
                 success:function(data)
                 {
